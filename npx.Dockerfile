# NPX-based lightweight Docker image for Tingly Box Nightly
# This image uses npm to install tingly-box-nightly globally, resulting in a smaller image size
...
# Install tingly-box-nightly globally during build (as root)
RUN npm install -g tingly-box-nightly@${TINGLY_VERSION}
...
RUN tingly-box-nightly version

# Health check
HEALTHCHECK --interval=30s --timeout=10s --start-period=10s --retries=3 \
    CMD tingly-box-nightly version || exit 1

# Default command: run tingly-box-nightly
CMD ["sh", "-c", "echo '======================================' && \
     echo '  Tingly Box Nightly is starting up...' && \
     ...
     echo '======================================' && \
     exec tingly-box-nightly start --host ${TINGLY_HOST} --port ${TINGLY_PORT}"]
