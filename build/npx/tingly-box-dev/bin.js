...
const BINARY_RELEASE_BRANCH = "dev";
...
function parseTransportVersion() {
	...
	let transportVersion = "dev"; // Default to dev
...
function validateTransportVersion(version) {
	...
	console.error(`Transport version must be either "dev", "latest", "v1.2.3", or "v1.2.3-prerelease1"`);
	process.exit(1);
}
...
(async () => {
	...
	// Use "tingly-box-dev" for caching to separate from nightly
	const tinglyBinDir = join(cacheDir(), "tingly-box-dev", branchName, "bin");
	...
	// The extracted binary path (no platform suffix in binary name)
    ...
    try {
        ...
        console.error(`\n❌ Tingly-Box-Dev execution failed`);
        ...
        if (errorCode) {
            ...
            switch (errorCode) {
                case 'ENOENT':
                    ...
                    console.error(`│     Try removing the cached binary: rm -rf "${join(cacheDir(), 'tingly-box-dev')}"`);
                    break;
                case 'EACCES':
        ...
        // Suggest retry
        console.error(`\n🔄 To retry, run: npx tingly-box-dev ${remainingArgs.join(' ')}`);
        console.error(`   Or clear cache first: rm -rf "${join(cacheDir(), 'tingly-box-dev')}"`);
    }
